...
def output_prediction(prediction_value: bool, best_model_name: str, aircraftid: str, date: str, tuple_to_predict: DataFrame):
    ...
    print(f"Using {best_model_name} for predicting tuple [{aircraftid}, {date}, {tuple_to_predict.select('sensor_mean_value').collect()[0][0]}, {tuple_to_predict.select('flighthours').collect()[0][0]}, {tuple_to_predict.select('delayedminutes').collect()[0][0]}, {tuple_to_predict.select('flightcycles').collect()[0][0]}]")
