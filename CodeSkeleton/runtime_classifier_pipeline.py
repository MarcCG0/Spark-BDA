...
def retrieve_dw_table(db_name: str, table_instance: str, session : SparkSession, table_name:str, columns: list[str], username:str, password:str) -> DataFrame: 
    ...
    try:
        ...
        raise ValueError(f"{Colors.RED}Failed to retrieve {table_name} table.{Colors.RESET}")
...
def get_sensor_data(session: SparkSession, aircraft_id: str, date: str) -> DataFrame:
    ...
    if aircraft_id is None or date is None:
        raise ValueError(f"{Colors.RED}Both aircraft_id and date must be provided{Colors.RESET}")
        
    df = df.where((df.aircraftid == aircraft_id) & (df.date == date))
    
    if df.isEmpty():
        raise ValueError(f"{Colors.RED}No tuple for the combination of date-aircraftid given by the user found in the database{Colors.RESET}")
...
def output_prediction(prediction_value: bool, best_model_name: str, aircraftid: str, date: str, tuple_to_predict: DataFrame):
    """Print the predicted value for the given input."""

    print(f"{Colors.GREEN}Using {best_model_name} for predicting tuple [{aircraftid}, {date}, {tuple_to_predict.select('sensor_mean_value').collect()[0][0]}, {tuple_to_predict.select('flighthours').collect()[0][0]}, {tuple_to_predict.select('delayedminutes').collect()[0][0]}, {tuple_to_predict.select('flightcycles').collect()[0][0]}]{Colors.RESET}")


    print(f"{Colors.GREEN}No maintenance predicted for the next 7 days{Colors.RESET}" if prediction_value == 0 else f"{Colors.GREEN}Unscheduled maintenance predicted in the next 7 days{Colors.RESET}")
...
def runtime_classifier_pipeline(spark: SparkSession, aircraftid: str, date: str, username: str, password: str):
    ...
    tuple_to_predict = prepare_data_before_prediction(filtered_sensor_data, aircraft_utilization, aircraftid, date)
    ...
    output_prediction(prediction_value, best_model_name, aircraftid, date, tuple_to_predict)
